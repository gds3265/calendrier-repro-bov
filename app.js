...
const DEFAULTS={heatWatchStart:18,heatWatchEnd:24,presumedPregnant:25,pregCheck:35,preCalving:285,term:295,primiparaAdvance:20,estiveAdvance:23,postpartumStart:30,postpartumWarn:45,postpartumLate:60,ivvWatch:400,ivvPoor:420};
...
async function clearLegacyPwaCaches(){
 try{
   if('caches' in window){const keys=await caches.keys();await Promise.all(keys.filter(k=>k.startsWith('repro-bovine')&&k!=='repro-bovine-v178').map(k=>caches.delete(k)))}
 }catch(_){}
}
...
function calvingDates(c){const xs=[];if(Array.isArray(c.calvingHistory))xs.push(...c.calvingHistory.filter(Boolean));if(c.lastCalving)xs.push(c.lastCalving);for(const e of events(c))if(e.type==='calving'&&e.date)xs.push(e.date);return [...new Set(xs)].sort()}
function realizedIVV(c){const ds=calvingDates(c);if(ds.length<2)return null;const last=ds.at(-1),prev=ds.at(-2),days=diffDays(last,prev);return days>0?{days,from:prev,to:last}:null}
function theoreticalIVV(c){const lc=lastCalving(c),b=pregnancyBasis(c);if(!lc||!b?.conception||b.source!=='service'||b.conception<=lc)return null;const calving=dateISO(addDays(b.conception,state.settings.term)),days=diffDays(calving,lc);if(days<=0)return null;const u=Math.max(0,Number(b.uncertaintyDays)||0);return {days,calving,service:b.conception,uncertainty:u,confirmed:!!b.confirmed,mode:b.sourceEvent?.mode||'natural'} }
function ivvClass(days){const poor=Math.max(Number(state.settings.ivvPoor)||420,Number(state.settings.ivvWatch)||400),watch=Math.min(Number(state.settings.ivvWatch)||400,poor);return days>=poor?'danger':days>=watch?'warn':'ok'}
function ivvLabel(c){const th=theoreticalIVV(c);if(th)return `IVV th. ${th.uncertainty?'~':''}${th.days} j${th.uncertainty?' ±'+th.uncertainty+' j':''}`;const real=realizedIVV(c);return real?`IVV réel ${real.days} j`:''}
...
function renderCows(){
  ...
  if(cowFilter==='postpartum')list=list.filter(c=>reproductiveStatus(c).key==='postpartum');if(cowFilter==='postabortion')list=list.filter(c=>reproductiveStatus(c).key==='postabortion');if(cowFilter==='ivv')list=list.filter(c=>{const x=theoreticalIVV(c);return x&&x.days>=Number(state.settings.ivvWatch||400)});
  ...
  list.sort((a,b)=>{
    ...
    if(sortMode==='ivv'){const ia=theoreticalIVV(a)?.days??-1,ib=theoreticalIVV(b)?.days??-1;if(ia!==ib)return ib-ia;return (a.workNumber||'').localeCompare(b.workNumber||'',undefined,{numeric:true});}
    if(sortMode==='calving'){
  ...
  $('#cowList').innerHTML=list.length?list.map(c=>{const s=reproductiveStatus(c), lc=lastCalving(c), nc=nextCalvingDate(c), p=prio.get(c.id), th=theoreticalIVV(c), rivv=realizedIVV(c); const under=isUnderAge(c)&&c.reproOverride!=='include'; const excluded=c.reproOverride==='exclude'; const badge=c.active===false?'Sortie':under?'Hors âge':excluded?'Exclue du suivi':s.label; const cls=c.active===false||under||excluded?'neutral':s.cls; const prioTxt=sortMode==='priority'&&p&&p.group<3?`⏱ ${esc(p.label)} ${frDate(p.date)} • `:''; return `<button class="card cow-card open-cow ${c.active===false?'inactive-card':''}" data-id="${esc(c.id)}"><span><span class="cow-name">${esc(c.name||'Sans nom')} · ${esc(c.workNumber)}</span><span class="cow-sub">${prioTxt}${nc?`🍼 mise bas présumée ${frDate(nc)} • `:''}${ageText(c.birthDate)}${lc?` • dernier vêlage ${frDate(lc)}`:''}${th?` • <strong class="ivv-${ivvClass(th.days)}">${esc(ivvLabel(c))}</strong>`:rivv?` • ${esc(ivvLabel(c))}`:''}${c.calvingCount?` • rang ${c.calvingCount}`:''}${c.reproOverride==='include'?' • inclusion forcée':''}${cowLocationText(c)?` • 📍 ${esc(cowLocationText(c))}`:''}${c.estiveActive?' • ⛰️ estive':''}</span></span><span class="badge ${cls}">${esc(badge)}</span></button>`}).join(''):`<div class="empty">Aucune vache trouvée.</div>`;
  bindCowOpen();
}
...
function openCow(id){
  ...
  ${thivv?`<div class="card ivv-card ${ivvClass(thivv.days)}"><strong>📊 IVV théorique : ${thivv.uncertainty?'~':''}${thivv.days} jours${thivv.uncertainty?' ±'+thivv.uncertainty+' j':''}</strong><div class="cow-sub">Calcul : dernier vêlage ${frDate(lastCalving(c))} → ${thivv.mode==='ai'?'IA':'saillie'} du ${frDate(thivv.service)} + terme réglé (${state.settings.term} j) = vêlage théorique ${frDate(thivv.calving)}.</div><div class="cow-sub">${thivv.days>=Number(state.settings.ivvPoor||420)?'🔴 Performance reproduction faible selon le seuil réglé.':thivv.days>=Number(state.settings.ivvWatch||400)?'🟠 IVV long : vache à surveiller pour le tri reproduction.':'🟢 IVV théorique sous le seuil de surveillance.'}${thivv.confirmed?' • Gestation confirmée':' • Saillie non encore confirmée'}</div></div>`:realivv?`<div class="card ivv-card ${ivvClass(realivv.days)}"><strong>📊 Dernier IVV réel : ${realivv.days} jours</strong><div class="cow-sub">Entre les vêlages du ${frDate(realivv.from)} et du ${frDate(realivv.to)}. Aucun IVV théorique courant calculable à partir d’une saillie observée.</div></div>`:''}
...
function saveCowForm(e){e.preventDefault(); const editId=$('#cowEditId').value, national=$('#cowNationalId').value.trim(), work=$('#cowWorkNumber').value.trim(); if(!work){alert('Le numéro de travail est obligatoire.');return}
 ...
 const data={id:newId,workNumber:work,name:$('#cowName').value.trim(),birthDate:$('#cowBirthDate').value,breed:$('#cowBreed').value.trim(),lastCalving:$('#cowLastCalving').value,calvingCount:Math.max(0,Number($('#cowCalvingCount').value)||0),reproOverride:$('#cowForceRepro').checked?'include':(c?.reproOverride==='exclude'?'exclude':''),calvingHistory:c?.calvingHistory||[]};
...
// --- Repro Bovine v1.7.7 : bilan reproduction (règles Audit bovin) ---
function monthsAt(birth,ref){const b=parseDate(birth),r=parseDate(ref);if(!b||!r)return null;let m=(r.getFullYear()-b.getFullYear())*12+r.getMonth()-b.getMonth();if(r.getDate()<b.getDate())m--;return Math.max(0,m)}
function allCalvingDates(c){return calvingDates(c).filter(Boolean).sort()}
function firstCalvingDate(c){const ds=allCalvingDates(c),rank=Number(c.calvingCount||0);return ds.length&&(!rank||ds.length>=rank)?ds[0]:''}
function ageFirstCalvingMonths(c){const d=firstCalvingDate(c);return d&&c.birthDate?monthsAt(c.birthDate,d):null}
function allIVVs(c){const ds=allCalvingDates(c),out=[];for(let i=1;i<ds.length;i++){const days=diffDays(ds[i],ds[i-1]);if(days>0)out.push({days,from:ds[i-1],to:ds[i],index:i})}return out}
function reportDateBounds(){
 const dates=[];for(const c of state.cows){if(c.birthDate)dates.push(c.birthDate);if(c.lastCalving)dates.push(c.lastCalving);for(const e of events(c))if(e.date)dates.push(e.date)}
 const end=dateISO(today());const min=dates.filter(Boolean).sort()[0]||dateISO(addDays(today(),-365));return {min,end};
}
function defaultReportPeriod(){const b=reportDateBounds(),end=b.end,start=dateISO(addDays(parseDate(end),-365));return {start:start<b.min?b.min:start,end}}
function presentAt(c,end){if(c.active!==false)return true;return !!c.exitDate&&c.exitDate>end}
...
function reportScore(c){
 ...
 if(avg!==null){if(avg>500){score-=30;reasons.push('IVV moyen >500 j : −30')}else if(avg>450){score-=20;reasons.push('IVV moyen 451–500 j : −20')}else if(avg>400){score-=10;reasons.push('IVV moyen 401–450 j : −10')}}
 if(max!==null){if(max>730){score-=15;reasons.push('IVV max >730 j : −15')}else if(max>500){score-=8;reasons.push('IVV max 501–730 j : −8')}}
 if(cs.dead){const pen=Math.min(30,cs.dead*12);score-=pen;reasons.push(`${cs.dead} veau(x) mort(s) <6 m : −${pen}`)}
 return {score:Math.max(0,score),reasons,afc,avg,max};
}
function scoreClass(n){return n>=75?'ok':n>=60?'warn':'danger'}
function pct(n,d){return d?Math.round(n/d*1000)/10:null}
function reportCowRows(start,end){
 return state.cows.filter(c=>presentAt(c,end)&&isReproEligible(c)).map(c=>{
   const ds=allCalvingDates(c),inPeriod=ds.filter(d=>d>=start&&d<=end),ivs=allIVVs(c),ivsPeriod=ivs.filter(x=>x.to>=start&&x.to<=end),first=ds[0]||'',last=ds.at(-1)||'',afc=ageFirstCalvingMonths(c),th=theoreticalIVV(c),sc=reportScore(c),age=monthsAt(c.birthDate,end);
   return {c,ds,inPeriod,ivs,ivsPeriod,first,last,afc,th,sc,age,calves:calfStats(c,start,end),calvesAll:calfStats(c),calved:inPeriod.length>0,primipara:(Number(c.calvingCount||ds.length)===1&&last&&last<=end),daysSince:last?diffDays(end,last):null};
 });
}
let reproProblemFilter='all',reproProblemSort='score';
function renderReproReport(){
 const host=$('#reproReport');if(!host)return;let start=$('#reproStart')?.value,end=$('#reproEnd')?.value;if(!start||!end){const p=defaultReportPeriod();start=p.start;end=p.end;if($('#reproStart'))$('#reproStart').value=start;if($('#reproEnd'))$('#reproEnd').value=end}if(start>end){[start,end]=[end,start];$('#reproStart').value=start;$('#reproEnd').value=end}
 const rows=reportCowRows(start,end),cows=rows.filter(r=>r.ds.length>0),heifers=rows.filter(r=>r.ds.length===0),over24=rows.filter(r=>r.age!==null&&r.age>24),over36=rows.filter(r=>r.age!==null&&r.age>36),calved24=over24.filter(r=>r.calved),calved36=over36.filter(r=>r.calved),calvedCows=cows.filter(r=>r.calved),prim=cows.filter(r=>r.primipara),allIvv=rows.flatMap(r=>r.ivsPeriod.map(x=>({...x,row:r}))),ivvVals=allIvv.map(x=>x.days),ivv1=rows.map(r=>r.ivs[0]).filter(x=>x&&x.to>=start&&x.to<=end).map(x=>x.days),afcs=cows.map(r=>r.afc).filter(x=>x!==null),mean=a=>a.length?Math.round(a.reduce((x,y)=>x+y,0)/a.length):null,min=a=>a.length?Math.min(...a):null,max=a=>a.length?Math.max(...a):null;
 const cls=[['≤ 400 j',ivvVals.filter(x=>x<=400).length,'ok'],['401–450 j',ivvVals.filter(x=>x>=401&&x<=450).length,'warn'],['451–500 j',ivvVals.filter(x=>x>=451&&x<=500).length,'warn'],['> 500 j',ivvVals.filter(x=>x>500).length,'danger']];
 const afcCls=[['< 28 mois',afcs.filter(x=>x<28).length,'ok'],['28–36 mois',afcs.filter(x=>x>=28&&x<=36).length,'warn'],['> 36 mois',afcs.filter(x=>x>36).length,'danger']];
 const noCalving24=over24.filter(r=>!r.calved),oldCalving=rows.filter(r=>r.daysSince!==null&&r.daysSince>400),thLong=rows.filter(r=>r.th&&r.th.days>400),calvesBorn=rows.reduce((a,r)=>a+r.calves.born,0),calvesDead=rows.reduce((a,r)=>a+r.calves.dead,0),calvesWithSire=rows.reduce((a,r)=>a+r.calves.withSire,0),calvesProbAI=rows.reduce((a,r)=>a+r.calves.probAI,0),repeatMort=rows.filter(r=>r.calvesAll.dead>=2),preg=rows.filter(r=>pregnancyBasis(r.c)?.confirmed).length,presumed=rows.filter(r=>{const b=pregnancyBasis(r.c);return b?.source==='service'&&!b.confirmed&&diffDays(end,b.conception)>=state.settings.presumedPregnant}).length;
 let probs=rows.filter(r=>r.sc.score<75||r.daysSince>400||(r.th&&r.th.days>400)||r.calvesAll.dead>=2);
 if(reproProblemFilter==='ivv')probs=probs.filter(r=>(r.ivs.length&&Math.max(...r.ivs.map(x=>x.days))>400)||(r.th&&r.th.days>400));
 if(reproProblemFilter==='calving')probs=probs.filter(r=>r.daysSince>400);
 if(reproProblemFilter==='afc')probs=probs.filter(r=>r.afc!==null&&r.afc>28);
 if(reproProblemFilter==='mortality')probs=probs.filter(r=>r.calvesAll.dead>=2);
 probs.sort((a,b)=>reproProblemSort==='score'?a.sc.score-b.sc.score:reproProblemSort==='ivv'?(b.th?.days||b.sc.max||0)-(a.th?.days||a.sc.max||0):reproProblemSort==='calving'?(b.daysSince||0)-(a.daysSince||0):reproProblemSort==='mortality'?b.calvesAll.dead-a.calvesAll.dead:String(a.c.workNumber||'').localeCompare(String(b.c.workNumber||''),undefined,{numeric:true}));
 const metric=(v,l,sub='')=>`<div class="metric"><strong>${v===null?'—':v}</strong><span>${l}${sub?`<br><small>${sub}</small>`:''}</span></div>`;
 host.innerHTML=`
 <div class="muted small">Période analysée : <strong>${frDate(start)}</strong> → <strong>${frDate(end)}</strong> • ${rows.length} femelle(s) suivie(s) présente(s)</div>
 <div class="repro-metrics">${metric(pct(calved24.length,over24.length)!==null?pct(calved24.length,over24.length)+' %':'—','Taux vêlage >24 m',`${calved24.length}/${over24.length}`)}${metric(pct(calved36.length,over36.length)!==null?pct(calved36.length,over36.length)+' %':'—','Taux vêlage >36 m',`${calved36.length}/${over36.length}`)}${metric(prim.length,'Primipares',calvedCows.length?`${pct(prim.length,cows.length)} % des vaches présentes`:'')}${metric(mean(ivvVals),'IVV moyen',ivvVals.length?`${min(ivvVals)}–${max(ivvVals)} j`:'aucun IVV sur la période')}${metric(mean(ivv1),'IVV1–IVV2 moyen',ivv1.length?`${ivv1.length} vache(s)`:'non calculable')}${metric(mean(afcs),'Âge 1er vêlage',afcs.length?`${min(afcs)}–${max(afcs)} mois`:'non calculable')}${metric(noCalving24.length,'Femelles >24 m sans vêlage','sur la période')}${metric(oldCalving.length,'Sans vêlage >400 j','à surveiller')}${metric(calvesDead,'Veaux morts <6 mois',calvesBorn?`${pct(calvesDead,calvesBorn)} % des ${calvesBorn} veaux nés`:'aucun veau sur la période')}${metric(calvesProbAI,'Veaux probablement issus d’IA',calvesWithSire?`${pct(calvesProbAI,calvesWithSire)} % des ${calvesWithSire} avec père renseigné`:'aucun père renseigné')}</div>
 <div class="repro-grid-2"><div class="card"><strong>Répartition des IVV</strong><div class="repro-class-list">${cls.map(x=>`<div class="repro-class-row"><span>${x[0]}</span><span class="badge ${x[2]}">${x[1]}</span></div>`).join('')}</div></div><div class="card"><strong>Âge au premier vêlage</strong><div class="repro-class-list">${afcCls.map(x=>`<div class="repro-class-row"><span>${x[0]}</span><span class="badge ${x[2]}">${x[1]}</span></div>`).join('')}</div></div></div>
 <div class="section-head"><h2>Situation actuelle</h2></div><div class="repro-metrics">${metric(preg,'Pleines confirmées')}${metric(presumed,'Supposées pleines')}${metric(thLong.length,'IVV théoriques >400 j')}${metric(heifers.length,'Génisses sans vêlage')}${metric(repeatMort.length,'Mères avec ≥2 morts <6 m','historique complet')}</div>
 <div class="section-head"><h2>Vaches à problème / à surveiller</h2><span class="muted small">score &lt;75, >400 j sans vêlage ou IVV théorique long</span></div>
 <div class="repro-toolbar"><label>Filtrer<select id="reproProblemFilter"><option value="all">Toutes les alertes</option><option value="ivv">IVV long</option><option value="calving">Sans vêlage >400 j</option><option value="afc">1er vêlage >28 mois</option><option value="mortality">≥2 veaux morts <6 mois</option></select></label><label>Trier<select id="reproProblemSort"><option value="score">Score repro (plus faible)</option><option value="ivv">IVV (plus long)</option><option value="calving">Dernier vêlage (plus ancien)</option><option value="mortality">Mortalité veaux (plus élevée)</option><option value="work">N° de travail</option></select></label></div>
 <div class="repro-table-wrap"><table class="repro-table"><thead><tr><th>Vache</th><th>Dernier vêlage</th><th>IVV réel</th><th>IVV théorique</th><th>1er vêlage</th><th>Morts <6 m</th><th>IA probable</th><th>Score</th><th></th></tr></thead><tbody>${probs.length?probs.map(r=>{const real=r.ivs.at(-1);return `<tr><td><strong>${esc(r.c.name||'Sans nom')}</strong><br><span class="muted">${esc(r.c.workNumber||'—')}</span></td><td>${r.last?frDate(r.last):'—'}${r.daysSince!==null?`<br><span class="muted">J+${r.daysSince}</span>`:''}</td><td>${real?real.days+' j':'—'}</td><td>${r.th?r.th.days+' j':'—'}</td><td>${r.afc!==null?Math.round(r.afc)+' mois':'—'}</td><td>${r.calvesAll.dead||'0'}</td><td>${r.calvesAll.probAI||'0'}</td><td><span class="score-pill ${scoreClass(r.sc.score)}">${r.sc.score}/100</span>${r.sc.reasons.length?`<br><span class="muted small">${esc(r.sc.reasons.join(' • '))}</span>`:''}</td><td><button type="button" class="ghost compact repro-open-cow" data-id="${esc(r.c.id)}">Voir</button></td></tr>`}).join(''):`<tr><td colspan="9" class="muted">Aucune vache ne correspond aux critères.</td></tr>`}</tbody></table></div>
 <div class="report-note muted small"><strong>Score reproduction :</strong> base 100 points. Pénalités identiques à l’Audit bovin : âge au 1er vêlage >28–36 m −8, >36 m −20 ; IVV moyen 401–450 j −10, 451–500 j −20, >500 j −30 ; IVV maximum 501–730 j −8, >730 j −15. ; mortalité veaux &lt;6 mois : −12 points par veau, plafonné à −30.</div>
 <div class="report-note muted small"><strong>IA probable :</strong> même heuristique que l’Audit bovin : père renseigné mais identifiant absent des mâles du registre importé. Ce résultat reste une estimation, pas une preuve d’IA.</div>`;
 const f=$('#reproProblemFilter'),so=$('#reproProblemSort');if(f){f.value=reproProblemFilter;f.onchange=()=>{reproProblemFilter=f.value;renderReproReport()}}if(so){so.value=reproProblemSort;so.onchange=()=>{reproProblemSort=so.value;renderReproReport()}}$$('.repro-open-cow').forEach(b=>b.onclick=()=>openCow(b.dataset.id));
}
...
document.addEventListener('DOMContentLoaded',()=>{
 ...
 if($('#reproStart'))$('#reproStart').onchange=renderReproReport; if($('#reproEnd'))$('#reproEnd').onchange=renderReproReport; if($('#reproLast12'))$('#reproLast12').onclick=setReportLast12; if($('#reproAllPeriod'))$('#reproAllPeriod').onclick=setReportAll;
